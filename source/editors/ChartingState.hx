...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var zoomList:Array<Float> = [0.0625, 0.125, 0.25, 0.5, 1, 2, 3, 4, 6, 8, 12, 16, 24, 32, 48, 64, 128, 192];
	var curZoom:Int = 4;
	...
	override function create()
	{
		...
		backdrop = new FlxBackdrop(Paths.image('checkeredBG'), #if (flixel_addons < "3.0.0") 1, 1, true, true, #else XY, #end 1, 1);
	...
	var sectionSelectedDropDown:FlxUIDropDownMenuCustom;
	...
	function addSectionUI():Void
	{
		...
		sectionSelectedDropDown = new FlxUIDropDownMenuCustom(150, 100, FlxUIDropDownMenuCustom.makeStrIdLabelArray(sectionListItems, true), function(selected:String)
		{
	...
	function addNoteUI():Void
	{
		...
		var copyButton:FlxButton = new FlxButton(10, noteTypeDropDown.y + 65, "Copy Note", function()
		{
		...
		if (FlxG.mouse.justPressed)
		{
			...
			else
			{
					...
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * getSectionBeats() * 4) * zoomList[curZoom])
				{
					...
					{
						addNote(curSelectedNote[0]
							+ (_song.notes[curSec].changeBPM ? 15000 / _song.notes[curSec].bpm : 15000 / _song.bpm) / stepperStackOffset.value,
							curSelectedNote[1]
							+ Math.floor(stepperStackSideOffset.value), currentType);
					}
					updateGrid();
					updateNoteUI();
				}
